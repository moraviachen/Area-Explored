...
public class MyBigInteger  implements Comparable<MyBigInteger> {   //  We will cover what "implements Comparable" means later
	...
	public MyBigInteger dividedBy( MyBigInteger  divisor ) throws Exception{
		...
		//check the size of the divisor
		int Dlength = divisor.coefficients.size();

		//create an empty arrayList to store the result
		MyBigInteger quotient = new MyBigInteger(this.base);


		int Tlength = this.coefficients.size();


		MyBigInteger Tnum=new MyBigInteger(this.base);
		int count=1;


		for(int i=0; i<Dlength; i++) {

			Tnum.coefficients.add(0,this.coefficients.get((int) (Tlength-count)));
		...
		MyBigInteger zero10 = new MyBigInteger(zero1,this.base);

		MyBigInteger zero20 = new MyBigInteger(zero2,this.base);
		MyBigInteger zero30 = new MyBigInteger(zero3,this.base);
		
		while(count-1 <= Tlength) {

			
			
			if(Tnum.coefficients.size()==divisor.coefficients.size()+1 && Tnum.coefficients.get(Tnum.coefficients.size()-1)==0) {
				Tnum.coefficients.remove(Tnum.coefficients.size()-1);
			}
			
			
			if(Tnum.compareTo(zero10)==0|| Tnum.compareTo(zero20)==0 || Tnum.compareTo(zero30)==0) {
				
				
				
				if(count-1!=Tlength && count-1<Tlength) {
				Tnum.coefficients.remove(0);
				quotient.coefficients.add(0,0);
				
				Tnum.coefficients.add(0,this.coefficients.get( (Tlength-count)));
				count ++;
				}else {
					Tnum.coefficients.remove(0);
					quotient.coefficients.add(0,0);
				...
				if(Tnum.compareTo(divisor)==-1 && count-1!=Tlength) {
					Tnum.coefficients.add(0,this.coefficients.get( (Tlength-count)));
					quotient.coefficients.add(0,0);
					count ++;
						...
						if(count-1==Tlength && Tnum.compareTo(divisor)==-1) {
							quotient.coefficients.add(0,0);
							break;
				...
				for(int i=1; i<this.base; i++){
					MyBigInteger te =divisor.timesSingleDigit(i);
					
					MyBigInteger hu = Tnum.minus(te);
					


					

					if(hu.compareTo(divisor)==-1) {
						...
						if(count-1<Tlength) {
							hu.coefficients.add(0,this.coefficients.get((int) (Tlength-count)));
							Tnum=hu;
							count++;

							/*if(Tnum.compareTo(zero20)==0&&Tlength-1!=count) {
								Tnum.coefficients.remove(0);
								quotient.coefficients.add(0,0);
		...
		int qL=quotient.coefficients.size();
		while(quotient.coefficients.get(qL-1)==0&&qL>1) {
			quotient.coefficients.remove(qL-1);
			qL=qL-1;
		}
	...
	public MyBigInteger convert(int newBase) throws Exception {
		...
		if(this.coefficients.isEmpty()) {
			throw new Exception("The coefficients of this big integer is empty");
		}
		
		if(newBase==0) {
			throw new Exception("The new base can't be 0");
		}
		
		if(newBase<0) {
			throw new Exception("The new base can't be negative");
		}
		...
		if(newBase==this.base) {
			...
			if(newBase>this.base) {
				int t = (int)remainder.coefficients.get(remainder.coefficients.size()-1);
				MyBigInteger fst=new MyBigInteger(t, newBase);


				result=result.plus(fst);

				for(int i =1; i<this.coefficients.size();i++) {
					result=result.timesSingleDigit(remainder.base);

					MyBigInteger temp=new MyBigInteger((int)remainder.coefficients.get(remainder.coefficients.size()-i-1),newBase);
				...
				for(int i=0; i<remainder.coefficients.size(); i++) {
					d=(int) (d+remainder.coefficients.get(i)*Math.pow(remainder.base, i));
				}

				while(d!=0) {
					result.coefficients.add((int)d%newBase);
					d=d/newBase;
				}*/
				...
				MyBigInteger ba = new MyBigInteger(""+newBase,remainder.base);

				
				while(!remainder.coefficients.isEmpty()) {
					
					
					//MyBigInteger temp = remainder.mod(ba);
					MyBigInteger hu = remainder.clone();
					//System.out.println("hu: "+hu);
					//System.out.println("ba: "+ ba);
					MyBigInteger ai=hu.dividedBy(ba);
					//System.out.println("hi");
					ai=ai.times(ba);
					//System.out.println(ai);
					int s = remainder.coefficients.size();
					
					if(remainder.coefficients.get(s-1).equals(0)) {
						remainder.coefficients.remove(s-1);
					}
					
					
					
					
					if(remainder.compareTo(ai)==-1) {
						break;
					}
					
					MyBigInteger temp = remainder.minus(ai);
					
					
					int x = temp.coefficients.get(0);
					result.coefficients.add(x);

					remainder=remainder.dividedBy(ba);
	...
	public ArrayList<MyBigInteger>  primeFactors() throws Exception {
		...
		if(this.compareTo(zero)==0) {
			throw new Exception("The coefficients of this big integer is zero");
		}
	...
	if(this.compareTo(one)==0) {
		throw new Exception("The coefficients of this big integer is one");
		}
		...
		MyBigInteger huh = this.convert(10);

		long x=0;
		for(int i=0; i<huh.coefficients.size();i++) {
			x=(long) (x+huh.coefficients.get(i)*Math.pow(10, i));
		}

		

		int ha=(int) Math.sqrt(x);

		//ArrayList<MyBigInteger> hu = primesToN(ha, 10);
		  ...
		  ArrayList<MyBigInteger> primes = primesToN(ha, this.base);
		  ...
		  MyBigInteger temp = remainder.mod(primes.get(i));
		  ...
		  remainder=remainder.dividedBy(primes.get(i));
		  MyBigInteger hu = remainder.clone();
			
			MyBigInteger ai=hu.dividedBy(primes.get(i));
			
			ai=ai.times(primes.get(i));
			//System.out.println(ai);
			int s = remainder.coefficients.size();
			...
			MyBigInteger haa = remainder.minus(ai);
		  temp = haa;
		...
		while(fst==0) {
			...
			MyBigInteger ai = new MyBigInteger(2,10);
			MyBigInteger a2 = ai.convert(this.base);
		...
		/*for(int i=2; i<hu.size(); i++) {
			int tem = 0;
			for(int j=0; j<hu.get(i).coefficients.size(); j++) {
				tem=(int) (tem+(int)hu.get(i).coefficients.get(j)*Math.pow(10, j));
			}
			...
			while(mod==0){
				mod=(int)(x%tem);
				x=(int) (x/tem);
				MyBigInteger temp = new MyBigInteger((int)tem,10);
				MyBigInteger ugh = temp.convert(this.base);
	...
	/*public static void main(String[] args) {
		try {
			MyBigInteger div = new MyBigInteger(24, 10);
			MyBigInteger divisor = new MyBigInteger(4,10);
			System.out.println(div.primeFactors());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}*/
